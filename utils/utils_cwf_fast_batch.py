...
def get_clean_mean_batch(source, basis, batch_size, from_clean_src = False):
    ...
    for l in range(0, batch_num):
        ...
        if from_clean_src:
            imgs_clean_l = source.images[batch_start[l] : batch_start[l] + batch_size_list[l]].asnumpy()
        else:
...
def get_clean_covar_batch(source, basis, mean_clean, batch_size, dtype, from_clean_src = False):
