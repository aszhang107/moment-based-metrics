...
from fourier_transform_utils import get_grid_of_radial_distances, get_inverse_fourier_transform, get_k_coordinate_of_each_pixel
...
atom_coeff_path = 'atom_coeffs_extended.json'
...
# def save_potential_to_mrc(string, voxel_size, N):
#     import mrcfile
#     with mrcfile.new('tmp.mrc') as mrc:
#     mrc.set_data(example_data)
#     # Spectrum on the regular cartesian Fourier grid.
#     protein_vol_ft = generate_molecule_spectrum_from_pdb_id(atom_group, voxel_size, N, from_atom_group= True)

#     # To visualize it, we can fourier transform and project it along axis = 0
#     protein_vol = get_inverse_fourier_transform(protein_vol_ft, voxel_size = voxel_size)
...
def generate_bag_of_atom_projection(grid_size, radius, voxel_size, N_atoms, atom_shape_fn):
    ...
    image = np.real(get_inverse_fourier_transform(ft_mol[grid_size//2], voxel_size = voxel_size))#.astype(np.float)
    return image


def get_fourier_transform_of_molecules_on_k_grid(atom_coords, weights, voxel_size, grid_size, eps = 1e-6, jax_backend = False):    
...
def get_fourier_transform_of_molecules_at_freq_coords(atom_coords, weights, voxel_size, freq_coords, eps = 1e-6, jax_backend = False):    

    # normalized_atom_coords = (atom_coords)/grid_size * (2*np.pi) / voxel_size
    normalized_atom_coords = atom_coords  * 2 * np.pi
    ft = finufft.nufft3d3(normalized_atom_coords[:,0], normalized_atom_coords[:,1], normalized_atom_coords[:,2], weights, freq_coords[:,0], freq_coords[:,1], freq_coords[:,2] ,  isign=-1, eps = eps)
    return ft
...
def compute_one_line_of_transforms(directory, molecule_options ):
    ...
    count = 0 
    r_dict_computed = False
    
    with open(directory + "/molecule_options.json", "w") as outfile:
        json.dump(molecule_options, outfile)
    
    for k in range(molecule_options["n_molecules"]):
        one_line_fp = directory + "/mol" + str(k) + "_online.npy"
        abs_spherical_average_filepath = directory + "/mol" + str(k) + "_average_abs.npy"    
        img = generate_synthetic_molecule_on_grid(radius = molecule_options["radius"], N = molecule_options["grid_size"], voxel_size = molecule_options["voxel_size"])
        one_line = img[:,0,0]
    
        with open(one_line_fp, 'wb') as f:
            np.save(f, one_line)
...
def center_atoms(atoms):
    ...
    masses = atoms.getMasses()
    weighted_coords = coords * np.expand_dims(masses, axis = 1)
