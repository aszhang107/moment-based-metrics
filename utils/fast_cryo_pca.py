...
class FastPCA:
    ...
    def _build(self):
        ...
        self.h_ctf = self.options["h_ctf"]
        self.h_idx = self.options["h_idx"]
        self.shifts = self.options["shifts"] #REMOVE LATER?
        ...
        if self.whiten:
            ...
            if self.single_filter and self.estimate_noise:
                ...
                for l in range(0, batch_num):
                    ...
                    if self.flip_sign:
                        if self.shifts is not None: # REMOVE THIS ?
                            imgs_noise_l = -self.src.images[batch_start[l]:batch_start[l]+batch_size_list[l]].shift(self.shifts[batch_start[l]:batch_start[l]+batch_size_list[l]]).asnumpy()
                        else:
    ...
    def denoise_images(self, mean_est=None, covar_est=None, denoise_options=None):
        ...
        clean_src = self.options["clean_src"]
        ...
        for k in range(denoise_df_ct):
            ...
            for l in range(k_batch_num):
                if return_error:
                    if clean_src != None:
                        imgs_clean_l = clean_src.images[k_batch_start[l]: k_batch_start[l] + k_batch_size_list[l]].asnumpy()
                    else:
